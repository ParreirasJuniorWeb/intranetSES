...
try {
  ...
  app.post(
    "/send-email",
    upload.array("file", 5), // deve ser "arquivos" se for esse nome no front-end <input type="file" name="arquivos" multiple />
    // nome deve coincidir
    async (req, res) => {
      ...
      <p>${message.replace(/\n/g, "<br>")}</p>
      <!------Preciso adicionar vários anexos neste e-mail e caso não haja nem um anexo um mensagem informado----->
      <p><strong>Anexos:</strong></p>
      <p>${
        attachedFile
          ? `Arquivo anexado: ${attachedFile.originalname}
      `
